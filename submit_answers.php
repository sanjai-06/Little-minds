...
if (!isset($data['depression'], $data['anxiety'], $data['stress'], $data['emotion'], $data['sleep'], $data['anger'])) {
    die(json_encode(['status' => 'error', 'message' => 'Incomplete data received']));
}

// Prepare and execute SQL query to store the data
$sql = "INSERT INTO answers (depression, anxiety, stress, emotion, sleep, anger) VALUES (?, ?, ?, ?, ?, ?)";
...
$stmt->bind_param(
    ...
    json_encode($data['anger']) // Changed to match the JavaScript key
);
